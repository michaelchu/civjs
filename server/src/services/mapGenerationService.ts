...
// Seeded random number generator for deterministic map generation per game
class SeededRandom {
  private seed: number;
...
export class MapGenerationService {
  ...
  private getDistanceFromCoast(
    ...
    for (let ox = 0; ox < dimensions.width; ox++) {
      for (let oy = 0; oy < dimensions.height; oy++) {
        if (!landMap[ox]?.[oy]) {
  ...
  public generateMap(gameId: string, settings: GameSettings): MapTile[] {
    // Initialize seeded random generator with unique game ID
    this.rng = new SeededRandom(
      gameId + settings.mapSize + Date.now().toString()
    );

    const dimensions = this.getMapDimensions(settings.mapSize);
    console.log(
      `Generating map for game ${gameId} with size ${settings.mapSize}: ${dimensions.width}x${dimensions.height}`
    );
    const tiles: MapTile[] = [];
  ...
  public generateStartingPositions(
    ...
    if (!this.rng) {
      this.rng = new SeededRandom('fallback' + Date.now().toString());
    }

    const positions: StartingPosition[] = [];
    const minDistance = Math.min(dimensions.width, dimensions.height) / 4;
    const attempts = 1000;

    for (let i = 0; i < playerCount; i++) {
      let position: StartingPosition | null = null;

      for (let attempt = 0; attempt < attempts; attempt++) {
        const candidate = {
          x: this.rng.nextInt(5, dimensions.width - 6),
          y: this.rng.nextInt(5, dimensions.height - 6),
        };
      ...
      // Fallback to distributed positions if random placement fails
      if (!position) {
        const angle = (i / playerCount) * 2 * Math.PI;
        const radius = Math.min(dimensions.width, dimensions.height) * 0.3;
        position = {
          x: Math.floor(dimensions.width / 2 + Math.cos(angle) * radius),
          y: Math.floor(dimensions.height / 2 + Math.sin(angle) * radius),
        };
      }
