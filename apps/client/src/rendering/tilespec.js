  ...
  switch (layer) {
    ...
    case LAYER_TERRAIN3:
      if (ptile != null) {
        ...
        sprite_array = sprite_array.concat(
          fill_irrigation_sprite_array(ptile, pcity)
        );
      }
      break;
    ...
    case LAYER_ROADS:
      if (ptile != null) {
        sprite_array = sprite_array.concat(
          fill_path_sprite_array(ptile, pcity)
        );
      }
      break;
    ...
    case LAYER_SPECIAL1:
      if (ptile != null) {
        ...
        sprite_array = sprite_array.concat(
          fill_layer1_sprite_array(ptile, pcity)
        );
    ...
    case LAYER_SPECIAL2:
      if (ptile != null) {
        sprite_array = sprite_array.concat(
          fill_layer2_sprite_array(ptile, pcity)
        );
      }
      break;
    ...
    case LAYER_UNIT:
      ...
      if (do_draw_unit && active_city == null) {
        ...
        sprite_array = sprite_array.concat(
          fill_unit_sprite_array(punit, stacked, backdrop)
        );
      }
    ...
    case LAYER_FOG:
      sprite_array = sprite_array.concat(
        fill_fog_sprite_array(ptile, pedge, pcorner)
      );
    ...
    case LAYER_SPECIAL3:
      if (ptile != null) {
        sprite_array = sprite_array.concat(
          fill_layer3_sprite_array(ptile, pcity)
        );
      }
      break;
        ...
        case MATCH_NONE: {
          ...
          if (dlp['dither'] == true) {
            for (var i = 0; i < num_cardinal_tileset_dirs; i++) {
              if (
                ts_tiles[tterrain_near[DIR4_TO_DIR8[i]]['graphic_str']] == null
              )
                continue;
              var near_dlp =
                tile_types_setup[
                  'l' + l + '.' + tterrain_near[DIR4_TO_DIR8[i]]['graphic_str']
                ];
              var terrain_near =
...
function fill_irrigation_sprite_array(ptile, pcity) {
  ...
  if (tile_has_extra(ptile, EXTRA_IRRIGATION) && pcity == null) {
    if (tile_has_extra(ptile, EXTRA_FARMLAND)) {
      result_sprites.push({
        key: tileset_extra_id_graphic_tag(EXTRA_FARMLAND),
      });
    } else {
      result_sprites.push({
        key: tileset_extra_id_graphic_tag(EXTRA_IRRIGATION),
      });
    }
  }
