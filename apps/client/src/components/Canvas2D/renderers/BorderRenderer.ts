...
export class BorderRenderer extends BaseRenderer {
  ...
  private tintedMasks = new Map<string, HTMLCanvasElement>();
  ...
  public render(state: RenderState, visibleTiles?: Tile[]): void {
    ...
    if (usesNativeLogicalProjection(map.topology_id ?? 0)) {
      this.renderNativeHexBorders(state, orderedTiles);
      return;
    }
  ...
  /**
   * Paint Hexemplio's three canonical edge elements with the two owner masks
   * generated by Freeciv for each edge. Processing S/E/SE once per tile covers
   * every six-sided edge without drawing the same mask twice.
   *
   * @reference reference/freeciv/client/mapview_common.h:121-185
   * @reference reference/freeciv/client/tilespec.c:5735-5846,7644-7678
   */
  private renderNativeHexBorders(state: RenderState, tiles: readonly Tile[]): void {
    const { map, players, viewport } = state;
    const edges = [
      {
        dx: 0,
        dy: 1,
        offsetX: -this.tileWidth / 4,
        offsetY: this.tileHeight / 4,
        masks: ['grid.borders.n', 'grid.borders.s'],
      },
      {
        dx: 1,
        dy: 0,
        offsetX: this.tileWidth / 4,
        offsetY: this.tileHeight / 4,
        masks: ['grid.borders.w', 'grid.borders.e'],
      },
      {
        dx: 1,
        dy: 1,
        ...
        masks: ['grid.borders.u', 'grid.borders.d'],
      },
    ] as const;

    for (const tile of tiles) {
      if (!tile.known || !this.isInViewport(tile.x, tile.y, viewport)) continue;
      ...
      for (const edge of edges) {
        ...
        const neighbor = position ? map.tiles[`${position.x},${position.y}`] : undefined;
        if (!neighbor?.known) continue;

        const owners = [this.getValidOwner(tile), this.getValidOwner(neighbor)] as const;
        if (owners[0] === owners[1]) continue;
        for (let side = 0; side < 2; side += 1) {
          const owner = owners[side];
          if (!owner || !players[owner]) continue;
          const mask = this.getTintedMask(
            edge.masks[side],
            this.getPlayerColors(owner, players).primary
          );
          if (mask) {
            this.ctx.drawImage(mask, screen.x + edge.offsetX, screen.y + edge.offsetY);
          }
        }
      }
    }
  ...
  private getValidOwner(tile: Tile): string | null {
    return tile.owner && tile.owner !== '255' ? tile.owner : null;
  }

  private getTintedMask(tag: string, color: string): HTMLCanvasElement | null {
    const cacheKey = `${tag}:${color}`;
    const cached = this.tintedMasks.get(cacheKey);
    if (cached) return cached;
    const source = this.tilesetLoader.getSprite(tag);
    ...
    this.tintedMasks.set(cacheKey, canvas);
    return canvas;
  }
