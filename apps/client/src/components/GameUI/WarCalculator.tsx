...
const statValue = (value: number | undefined): string => (value === undefined ? '—' : `${value}`);
...
const outcomeFor = (
  ...
  if (attacker > defender * 1.2)
    return { label: 'Attacker advantage', className: 'text-emerald-200' };
...
export const WarCalculator: React.FC<WarCalculatorProps> = ({
  ...
  const availableUnits = useMemo(
    () =>
      Object.values(units).filter(unit => unit.attack !== undefined || unit.defense !== undefined),
    [units]
  );
  ...
  useEffect(() => {
    if (!availableUnits.some(unit => unit.id === attackerId))
      setAttackerId(ownUnits[0]?.id ?? availableUnits[0]?.id ?? '');
    if (!availableUnits.some(unit => unit.id === defenderId))
      setDefenderId(opposingUnits[0]?.id ?? availableUnits[1]?.id ?? '');
  ...
  const ratio =
    attackerPower !== null && defenderPower && defenderPower > 0
      ? (attackerPower / defenderPower).toFixed(2)
      : '—';
      ...
      <HudDialogContent>
        <DialogHeader>
          ...
          <DialogDescription className="text-slate-400">
            Compare visible unit combat values before committing an attack. The server remains
            authoritative for the actual result.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-5">
          {availableUnits.length === 0 ? (
            <div className="rounded-xl border border-dashed border-white/15 bg-white/5 p-6 text-sm text-slate-400">
              Combat stats are not available in the current unit snapshot. Refresh the map snapshot
              or wait for the server combat-data update.
            </div>
          ) : (
            <>
              <section className="grid gap-3 md:grid-cols-2" aria-label="Combat participants">
                <UnitSelect
                  label="Attacker"
                  value={attackerId}
                  units={ownUnits.length ? ownUnits : availableUnits}
                ...
                <UnitSelect
                  label="Defender"
                  value={defenderId}
                  units={opposingUnits.length ? opposingUnits : availableUnits}
              ...
              {attacker && defender ? (
                ...
                >
                  ...
                  <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-5">
                    ...
                    <span className={`text-center text-sm font-semibold ${outcome.className}`}>
                      {outcome.label}
                    </span>
                ...
                <div className="rounded-xl border border-dashed border-white/15 bg-white/5 p-5 text-sm text-slate-400">
                  Select two units with combat data to compare them.
                </div>
              )}

              <section
                className="rounded-xl border border-amber-300/20 bg-amber-300/10 p-4"
                aria-label="Combat estimate limitations"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-5 text-amber-100/70">
                      This comparison uses base attack/defense, firepower, and current health.
                      Terrain, fortification, veteran modifiers, defenders in a stack, bombardment,
                      and ruleset-specific combat bonuses are not yet exposed to the calculator. No
                      win probability is implied.
                    </p>
                  </div>
                </div>
              </section>
...
const UnitSelect: React.FC<{
  ...
  <label className="rounded-xl border border-white/10 bg-white/5 p-3">
    ...
    <select
      ...
      className="mt-2 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-sm text-slate-100 outline-none focus:border-cyan-300/50 focus:ring-2 focus:ring-cyan-300/20"
    >
      {units.map(unit => (
        <option key={unit.id} value={unit.id}>
          {formatName(unit.unitTypeId)} · {unit.x},{unit.y}
        </option>
      ))}
    </select>
  </label>
...
const CombatCard: React.FC<{
  ...
  const accentClass =
    accent === 'cyan' ? 'text-cyan-200 border-cyan-300/20' : 'text-violet-200 border-violet-300/20';
  return (
    <div className={`rounded-xl border bg-white/5 p-4 ${accentClass}`}>
      ...
      <div className="mt-2 flex flex-wrap gap-1.5 text-[10px]">
        <span className="rounded border border-violet-300/20 bg-violet-300/10 px-1.5 py-0.5 text-violet-200">
          Veteran level {unit.veteranLevel}
        </span>
        <span
          className={`rounded border px-1.5 py-0.5 ${unit.fortified ? 'border-cyan-300/25 bg-cyan-300/10 text-cyan-200' : 'border-white/10 bg-white/5 text-slate-500'}`}
        >
          {unit.fortified ? 'Fortified' : 'Not fortified'}
        </span>
      </div>
      ...
      <div className="mt-3 border-t border-white/10 pt-2 text-[10px] text-slate-500">
        ...
        <span className="font-semibold tabular-nums text-slate-200">
          {power === null ? 'Unavailable' : power.toFixed(1)}
        </span>
      </div>
    </div>
  );
