  ...
  const rows = React.useMemo<DemographicRow[]>(() => {
    ...
    return activePlayers.map(player => {
      ...
      return {
        ...
        territory: Object.values(tiles).filter(tile => tile.owner === player.id && (tile.known || tile.visible)).length,
      ...
      <DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto border-white/15 bg-slate-900 text-white">
        <DialogHeader>
          ...
          <DialogDescription className="text-slate-400">
            Compare visible empire scale and economy. Territory and unit counts reflect information currently known to you.
          </DialogDescription>
        </DialogHeader>

        {rows.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-white/5 p-6 text-sm text-slate-400">Demographic data is not available yet.</div>
        ) : (
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4" role="group" aria-label="Demographic metric">
            ...
            <section aria-labelledby="demographic-bars-heading" className="rounded-xl border border-white/10 bg-slate-950/50 p-4">
              ...
              <div className="space-y-3">
                {[...rows].sort((left, right) => rowMetric(right, metric) - rowMetric(left, metric)).map(row => {
                  const value = rowMetric(row, metric);
                  const width = `${Math.max((value / maxMetric) * 100, value > 0 ? 4 : 0)}%`;
                  return (
                    <div key={row.player.id} className="grid grid-cols-[minmax(6rem,9rem)_1fr_3rem] items-center gap-3 text-xs">
                      <span className="truncate text-slate-300">{formatNation(row.player.nation)}</span>
                      <div className="h-2 overflow-hidden rounded-full bg-white/10">
                        <div className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-violet-400 transition-all" style={{ width }} />
                      </div>
                      <span className="text-right font-semibold tabular-nums text-slate-100">{value}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <section aria-labelledby="demographic-table-heading">
              <div className="mb-2 flex items-center gap-2">
                ...
                <h3 id="demographic-table-heading" className="text-sm font-semibold text-slate-100">Empire comparison</h3>
              </div>
              <Table className="border-white/10">
                <TableHeader className="bg-slate-800">
                  <TableRow className="border-white/10 hover:bg-slate-800">
                    ...
                    <TableHead className="text-right text-slate-300"><span className="inline-flex items-center gap-1"><FlaskConical className="h-3.5 w-3.5" aria-hidden="true" /> Science</span></TableHead>
                    <TableHead className="text-right text-slate-300"><span className="inline-flex items-center gap-1"><Coins className="h-3.5 w-3.5" aria-hidden="true" /> Gold</span></TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {rows.map(row => (
                    <TableRow key={row.player.id} className={row.player.id === currentPlayerId ? 'bg-cyan-400/10' : 'border-white/10'}>
                      <TableCell className="font-medium text-slate-100">{formatNation(row.player.nation)}</TableCell>
                      <TableCell className="text-right tabular-nums text-slate-300">{row.population}</TableCell>
                      <TableCell className="text-right tabular-nums text-slate-300">{row.cities}</TableCell>
                      <TableCell className="text-right tabular-nums text-slate-300">{row.units}</TableCell>
                      <TableCell className="text-right tabular-nums text-slate-300">{row.territory || '—'}</TableCell>
                      <TableCell className="text-right tabular-nums text-slate-300">{row.science >= 0 ? `+${row.science}` : row.science}</TableCell>
                      <TableCell className="text-right tabular-nums text-slate-300">{row.gold >= 0 ? `+${row.gold}` : row.gold}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
              <p className="mt-2 text-[10px] text-slate-500">Technology counts are intentionally limited to your researched catalogue; per-nation technology intelligence is not currently exposed by the backend.</p>
            </section>
          </div>
        )}
      </DialogContent>
