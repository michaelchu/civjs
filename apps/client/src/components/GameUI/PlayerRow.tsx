...
import type { Nation, PlayerNationInfo, DiplomaticState } from '@civjs/shared';
...
export const PlayerRow: React.FC<PlayerRowProps> = ({
  ...
  const { getPlayerDiplomaticState, getPlayerEmbassyStatus, getPlayerSharedVisionStatus } =
    useGameStore();
  ...
  const getDiplomaticStateDisplay = (state: DiplomaticState | null) => {
    ...
    const info = stateMap[state as keyof typeof stateMap];
  ...
  const getPlayerState = () => {
    ...
    if (!player.isHuman && player.turnsIdle && player.turnsIdle > 1) {
      return (
        <span className="text-xs text-muted-foreground">Idle for {player.turnsIdle} turns</span>
      );
    }
    ...
    <TableRow className={getRowClassName()} onClick={onClick}>
      ...
      <TableCell>
        <div className="flex items-center space-x-2">
          <span
            className={cn('font-medium', isCurrentPlayer && 'text-blue-600 dark:text-blue-400')}
          >
            {player.playerName}
          </span>
          {isCurrentPlayer && (
      ...
      {/* Nation */}
      <TableCell>
        <span className="cursor-help" title={nation.legend}>
          {nation.adjective}
        </span>
      </TableCell>
      ...
      {!isObserver && (
        <TableCell>
          <span className="text-sm">{player.attitude || '-'}</span>
        </TableCell>
      )}

      {/* Score */}
      <TableCell>
        <span className="font-mono text-sm">{formatScore(player.score)}</span>
      </TableCell>
      ...
      {!isObserver && <TableCell>{getDiplomaticStateDisplay(diplomaticState)}</TableCell>}

      {/* Embassy (not for observers) */}
      {!isObserver && (
        <TableCell>{getEmbassyDisplay(embassyStatus?.hasEmbassy || false)}</TableCell>
      )}

      {/* Shared Vision (not for observers) */}
      {!isObserver && <TableCell>{getSharedVisionDisplay()}</TableCell>}
      ...
      {/* Player State */}
      <TableCell>{getPlayerState()}</TableCell>
    </TableRow>
  );
};
