    ...
    >
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center justify-between text-xs">
          <span className="opacity-90">Cost: {cost}</span>
          {isCurrent && progress > 0 && <span className="opacity-90">{progress}%</span>}
        </div>
        ...
        {/* Research indicators */}
        {isResearched && <div className="text-xs text-green-700 font-medium">✓ Researched</div>}

        {isGoal && !isCurrent && <div className="text-xs text-blue-900 font-medium">→ Goal</div>}
      </div>
