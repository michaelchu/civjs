...
var goto_colors_rally = {
  ...
  2: ['40,0,10,1', '240,230,230,1', '54,0,10,1', '255,143,155,1'],
}; // persistent rally path
...
function mapview_cycle_city_display_mode() {
  ...
  if (city_map_display_mode >= last) city_map_display_mode = 0;
...
**************************************************************************/
const not_large_screen = () => {
  // helps figure out a medium screen like 1376 x 768
...
function init_sprites() {
  $.blockUI({
    ...
    message:
      "<h1 style='text-align:center;color:#ccc'>Loading..." +
      "<br><img src='/images/loading.gif'><br>" +
      "<font id='load_progress' size='1'>" +
      loaded_images +
      '/' +
      tileset_image_count +
      ' complete' +
...
function mapview_put_scaled_image(pcanvas, tag, canvas_x, canvas_y, scale) {
  ...
  pcanvas.drawImage(
    ...
    canvas_x,
    canvas_y,
    sprites[tag].width * scale,
    sprites[tag].height * scale
  );
}
  ...
  switch (city_map_display_mode) {
    ...
    case CMDM_FINISHED:
      ...
      if (num === undefined || num > 999) num = '';
        ...
        if (pcity['buy_cost']) {
          // still has shields though, show diff. color
          col = 'rgb(96,255,255)';
        }
...
function mapview_put_city_bar(pcanvas, city, canvas_x, canvas_y) {
  ...
  if (draw_city_mood) {
    if (client.conn.playing != null && !client_is_observer()) {
      if (
        city['owner'] == client.conn.playing.playerno &&
        city['ppl_happy'] != null &&
        city['ppl_content'] != null &&
        city['ppl_unhappy'] != null
      ) {
        var city_state = get_city_state(city);
        happy_people = city['ppl_happy'][FEELING_FINAL];
        content_people = city['ppl_content'][FEELING_FINAL];
        unhappy_angry_people =
          city['ppl_unhappy'][FEELING_FINAL] + city['ppl_angry'][FEELING_FINAL];
        ...
        if (
          happy_people >= city['size'] * 0.4999 &&
          unhappy_angry_people == 0 &&
          city['size'] >= city_celebrate_size(city)
        ) {
          ...
          // case: city going into disorder
          if (mood_text == celeb) {
            // if losing celebration, invert size color and size shadow
            size_shadow_color = 'rgba(128,128,128,1)';
            size_color = lose_celeb_color;
            lose_celeb = 1;
          }
        } else {
          // case handling: city will go into peace next turn
          if (mood_text == celeb) {
            // if losing celebration, invert size color and size shadow
            size_shadow_color = 'rgba(128,128,128,1)';
            size_color = lose_celeb_color;
  ...
  if (draw_city_airlift_counter) {
    ...
    var src_capacity =
      game_info['airlifting_style'] & SRC_UNLIMITED ? infinity_symbol : city['airlift'];
    if (src_capacity < 0) src_capacity = 0;

    if (client.conn.playing != null && !client_is_observer()) {
      if (city['owner'] == client.conn.playing.playerno) {
        if (game_info['airlift_dest_divisor'] == 0) {
          // if no dest_divisor, there is one counter for both source and dest
          ...
          var airlift_receive_max_capacity = Math.round(
            city['size'] / game_info['airlift_dest_divisor']
          );

          if (game_info['airlifting_style'] & DEST_UNLIMITED)
            airlift_receive_text = infinity_symbol;
          // else destination airlifts allowed = population of city / airliftdivisor, rounded to nearest whole number:
          else
            airlift_receive_text = Math.max(
              0,
              city['airlift'] + airlift_receive_max_capacity - effects[1][0]['effect_value']
            );

          airlift_text =
            city['airlift'] > 0 ||
            airlift_receive_text == infinity_symbol ||
            src_capacity == infinity_symbol ||
  ...
  pcanvas.drawImage(
    ...
    canvas_x - Math.floor(txt_measure.width / 2) - 44,
    canvas_y - 17
  );
...
*/
  ...
  pcanvas.fillText(
    size,
    canvas_x + Math.floor(txt_measure.width / 2) + 10 - shadow_offset_fix,
    canvas_y + 1 - shadow_offset_fix
  );
...
function mapview_put_border_line(pcanvas, dir, color, color2, color3, canvas_x, canvas_y) {
  ...
  if (draw_moving_borders) {
    ...
    if (border_anim > 24 * border_anim_delay) border_anim = 0;
  }
...
function enable_mapview_slide(ptile) {
  ...
  if (
    (dx == 0 && dy == 0) ||
    mapview_slide['active'] ||
    Math.abs(dx) > mapview['width'] ||
    Math.abs(dy) > mapview['height']
  ) {
