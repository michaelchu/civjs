...
function mapview_mouse_click(e) {
  ...
  if (rightclick) {
    ...
    if (
      mouse_click_mod_key.shiftKey &&
      mouse_click_mod_key.ctrlKey &&
      !mouse_click_mod_key.altKey
    ) {
      ...
      !e.shiftKey &&
      !e.altKey &&
      !e.ctrlKey && // mod-keys don't trigger these actions
      (!map_select_active || !map_select_setting_enabled)
    ) {
      // in drag-select mode we don't it either
      context_menu_active = true;
      recenter_button_pressed(mouse_x, mouse_y);
    } else {
      // map_select_active from right-click-drag: button release means select and clean up
    ...
    if (map_select_active) {
      // button up = release from this mode, select all units in box
      context_menu_active = false;
      map_select_units(mouse_x, mouse_y);
      map_select_active = false;
      map_select_check = false;
      ...
      // PROCESS NORMAL LEFT CLICK HERE
...
function mapview_mouse_down(e) {
  ...
  } else if ($('.context-menu-list').is(':visible')) {
    // Context menu click-to-leave functionality for non-touch devices
  ...
  if (!rightclick && !middleclick) {
    ...
    if (!mouse_click_mod_key['shiftKey'] || metaKey(mouse_click_mod_key)) {
      // including metaKey makes mouse-down select the unit, so mouseup tells help-click which unit is selected
      check_mouse_drag_unit(canvas_pos_to_tile(mouse_x, mouse_y));
      // initial condition for possibly starting map drag mode
      if (
        !map_select_check && // no map drag if we're in selection rectangle or leaving it
        ...
        !came_from_context_menu
      ) {
        // we don't start a drag if coming out of context menu
  ...
  if (goto_active && current_focus.length > 0) {
    ...
    if (ptile != null) {
      for (var i = 0; i < current_focus.length; i++) {
        ...
        if (
          goto_request_map[current_focus[i]['id'] + ',' + ptile['x'] + ',' + ptile['y']] == null
        ) {
...
function recenter_button_pressed(canvas_x, canvas_y) {
  ...
  if (
    ptile != null &&
    ptile['y'] > map['ysize'] - map_scroll_border &&
    map['xsize'] > big_map_size &&
    map['ysize'] > big_map_size
  ) {
    ptile = map_pos_to_tile(ptile['x'], map['ysize'] - map_scroll_border);
  }
  if (
    ptile != null &&
    ptile['y'] < map_scroll_border &&
    map['xsize'] > big_map_size &&
    map['ysize'] > big_map_size
  ) {
  ...
  if (can_client_change_view() && ptile != null && orig_tile != null) {
    ...
    if (!client_is_observer() && sunit != null && sunit['owner'] == client.conn.playing.playerno) {
...
function handle_info_text_message(packet) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (re !== undefined) {
      ...
      if (pplayer != null && (client.conn.playing == null || pplayer != client.conn.playing)) {
        lines[i] =
          split_txt[1] +
  ...
  message = message.replace(
    /<br>\n<b>Hostile/g,
    " <b class='black_shadow' style='color:d85454'>Hostile"
  );
  message = message.replace(
    /<br>\nHostile/g,
    " <b class='black_shadow' style='color:d85454'>Hostile</b>"
  );
  message = message.replace(/<b>Hostile/g, "<b class='black_shadow' style='color:d85454'>Hostile");

  message = message.replace(
    /<br>\n<b>Friendly/g,
    " <b class='black_shadow' style='color:#1b1'>Friendly"
  );
  message = message.replace(
    /<br>\nFriendly/g,
    " <b class='black_shadow' style='color:#1b1'>Friendly</b>"
  );
  message = message.replace(/<b>Friendly/g, "<b class='black_shadow' style='color:#1b1'>Friendly");

  message = message.replace(
    /<br>\n<b>Neutral/g,
    " <b class='black_shadow' style='color:#40a4d6'>Neutral"
  );
  message = message.replace(
    /<br>\nNeutral/g,
    " <b class='black_shadow' style='color:#40a4d6'>Neutral</b>"
  );
  message = message.replace(/<b>Neutral/g, "<b class='black_shadow' style='color:#40a4d6'>Neutral");

  message = message.replace(
    /<br>\n<b>Mysterious/g,
    " <b class='black_shadow' style='color:#9072de'>Mysterious"
  );
  message = message.replace(
    /<br>\nMysterious/g,
    " <b class='black_shadow' style='color:#9072de'Mysterious</b>"
  );
  message = message.replace(
    /<b>Mysterious/g,
    "<b class='black_shadow' style='color:#9072de'>Mysterious"
  ...
  message = message.replace(
    /<br>\n<b>Peaceful/g,
    " <b class='black_shadow' style='color:#0b8'>Peaceful"
  );
  message = message.replace(
    /<br>\nPeaceful/g,
    " <b class='black_shadow' style='color:#0b8'Peaceful</b>"
  );
  ...
  message = message.replace(
    /11 turn cease-fire/g,
    "11 turn <b class='black_shadow' style='color:#790'>Cease-fire</b>"
  );
  // show warning colour for 1 turn left on cease-fire
  message = message.replace(
    /1 turn cease-fire/g,
    "<b class='black_shadow' style='color:#b50'><u>1 turn</u> Cease-fire</b>"
  );
  // standard cease-fire
  message = message.replace(
    /turn cease-fire/g,
    "turn <b class='black_shadow' style='color:#790'>Cease-fire</b>"
  );
