...
var UO_dx = [],
  UO_dy = []; // Unit graphic delta.
var UO_sx = [],
  UO_sy = []; // Shield graphic
var UO_vx = [],
  UO_vy = []; // Vet badge graphic
var UO_mx = [],
  UO_my = []; // Multi-unit graphic (stacked "+" icon)
...
const CITY_REPLACED_BY_WONDER_MASK =
  CITY_HANGING_GARDEN |
  CITY_ANGKOR_WAT |
  CITY_MAUSOLEUM |
  CITY_ZIGGURAT |
  CITY_PYRAMID |
  CITY_CITADEL; /* | CITY_CHICHEN_ITZA*/
  ...
  switch (layer) {
    case LAYER_TERRAIN1:
      if (ptile != null) {
        ...
        sprite_array = sprite_array.concat(
    ...
    case LAYER_SPECIAL1:
      if (ptile != null) {
        ...
        if (tile_has_extra(ptile, EXTRA_OIL_WELL)) {
          sprite_array.push({ key: tileset_extra_id_graphic_tag(EXTRA_OIL_WELL) });
        }
        ...
        if (tile_has_extra(ptile, EXTRA_POLLUTION)) {
          sprite_array.push({ key: tileset_extra_id_graphic_tag(EXTRA_POLLUTION) });
        ...
        if (tile_has_extra(ptile, EXTRA_FALLOUT)) {
          sprite_array.push({ key: tileset_extra_id_graphic_tag(EXTRA_FALLOUT) });
        }
        // (Optional) frontier-facing flags on borders
        if (draw_border_flags) sprite_array = sprite_array.concat(get_frontier_flag_sprites(ptile));
      }
      break;
    ...
    case LAYER_UNIT:
      var do_draw_unit =
        punit != null &&
        (draw_units || ptile == null || (draw_focus_unit && unit_is_in_focus(punit)));
      var is_stacked = false; // stacked units get a "+" icon drawn over them

      if (do_draw_unit && active_city == null) {
        is_stacked =
          ptile['units'] != null && ptile['units'].length > 1 ? ptile['units'].length : false;
      ...
      if (ptile != null && explosion_anim_map[ptile['index']] != null) {
        ...
        if (explode_step <= 1) {
          ...
          sprite_array.push({ key: key_prefix + frame, offset_x: xo, offset_y: yo });
    ...
    */
    ...
    case LAYER_CITYBAR:
      if (draw_city_output && active_city == null && ptile != null && ptile['worked'] > 0) {
        ...
        if (acity) {
          //might be undefined
          if (acity['food_output'] != null) {
            //this checks it's not foreign city with null info
    ...
    case LAYER_GOTO:
      ...
      if (ptile != null && ptile['nuke'] > 0) {
        ...
        sprite_array.push({ key: 'explode.nuke', offset_x: -45, offset_y: -45 });
      }
  ...
  switch (dlp['sprite_type']) {
    case CELL_WHOLE: {
      switch (dlp['match_style']) {
        case MATCH_NONE: {
          ...
          if (dlp['dither'] == true) {
            for (var i = 0; i < num_cardinal_tileset_dirs; i++) {
              ...
              var near_dlp =
                tile_types_setup['l' + l + '.' + tterrain_near[DIR4_TO_DIR8[i]]['graphic_str']];

              var terrain_near =
                pterrain['graphic_str'] == 'arctic' && !(near_dlp['dither'] == true)
                  ? 'swamp' // hard-coded for now, but dithers so much nicer into shore graphics
                  : near_dlp['dither'] == true
        ...
        case MATCH_SAME: {
          ...
          var gfx_key =
            't.l' + l + '.' + pterrain['graphic_str'] + '_' + cardinal_index_str(tileno);
    ...
    case CELL_CORNER: {
      ...
      for (var i = 0; i < NUM_CORNER_DIRS; i++) {
        ...
        var m = [
          'l' + l + '.' + tterrain_near[dir_ccw(dir)]['graphic_str'] in tile_types_setup
            ? tile_types_setup['l' + l + '.' + tterrain_near[dir_ccw(dir)]['graphic_str']][
                'match_index'
              ][0]
            : -1,
          'l' + l + '.' + tterrain_near[dir]['graphic_str'] in tile_types_setup
            ? tile_types_setup['l' + l + '.' + tterrain_near[dir]['graphic_str']]['match_index'][0]
            : -1,
          'l' + l + '.' + tterrain_near[dir_cw(dir)]['graphic_str'] in tile_types_setup
        ...
        result_sprites.push({
          key: cellgroup_map[pterrain['graphic_str'] + '.' + array_index] + '.' + i,
          offset_x: x,
          offset_y: y,
        });
      }
...
function fill_unit_sprite_array(punit, num_stacked) {
  ...
  */
  ...
  if (unit_offset['x'] == 0 && unit_offset['y'] == 0) {
    // if unit is moving, don't draw these
    // Move point bar
    ...
    if (num_stacked) {
      ...
      if (!UO_sy[id] && draw_stacked_unit_mode & dsum_RING) {
        // y-aligned shield doesn't get a ring for now.
        if (UO_sx[id] > 0) {
          // right-aligned shield:
          result.push({
            key: 'unit.stk_shld_r',
      ...
      if (draw_stacked_unit_mode & dsum_SMALL) {
        ...
        // normal mode has custom offsets
...
function fill_stacked_in_base_sprite_array(punit, num_stacked) {
  ...
  if (!(draw_stacked_unit_mode & dsum_SMALL)) {
    //// regular mode has custom offsets
    var id = punit['type'];
    const mx = UO_mx[id];
    const my = UO_my[id];
    stacked['offset_x'] += mx;
...
function get_city_invalid_worked_sprite() {
  return { key: 'grid.unavailable', offset_x: 0, offset_y: 0 };
}
...
function get_user_mark_sprite(index) {
  return { key: USER_MARKS[index], offset_x: 0, offset_y: 0 };
}
...
function get_frontier_flag_sprites(ptile) {
  ...
  if (border_dirs[DIR8_EAST] && drawn_dirs[DIR8_EAST] === undefined) drawn_dirs[DIR8_EAST] = true;
  if (border_dirs[DIR8_WEST] && drawn_dirs[DIR8_WEST] === undefined) drawn_dirs[DIR8_WEST] = true;
...
function get_border_line_sprites(ptile) {
  ...
  for (var i = 0; i < num_cardinal_tileset_dirs; i++) {
    ...
    if (
      ...
      ptile['owner'] != checktile['owner'] &&
      ptile['owner'] !=
        255 /* 255 is a special constant indicating that the tile is not owned by anyone. */ &&
      players[ptile['owner']] != null
    ) {
      var pnation = nations[players[ptile['owner']]['nation']];
      result.push({
        ...
        dir: dir,
        color: pnation.color,
        color2: pnation.color2,
        color3: pnation.color3,
      });
    }
  }
  return result;
...
function get_html_nation_shield_sprite(nation) {
  ...
  return (
    ...
    scale +
    '); background-position:-' +
    shield_sprite['tileset-x'] +
    'px -' +
    shield_sprite['tileset-y'] +
    'px;  width: ' +
    shield_sprite['width'] +
    'px;height: ' +
    shield_sprite['height'] +
    'px;' +
    ' content-align: left;' +
...
function get_unit_stack_sprite(stacksize) {
  if (draw_stacked_unit_mode & dsum_SMALL) {
    //// alternate small mode of showing stack near hpbar
    return {
      key: 'unit.stack' + stacksize + '',
...
function get_html_hp_sprite(punit, unit_panel_pos) {
  ...
  return (
    "<div style='pointer-events: none; margin-left:" +
    left_adj +
    'px; margin-top:' +
    top_adj +
    'px; margin-right: -96px; float:left; content-align:left;' +
    'background: transparent url(' +
    hp_sprite['image-src'] +
    ');transform: scale(0.5); background-position:-' +
    hp_sprite['tileset-x'] +
    'px -' +
    hp_sprite['tileset-y'] +
    'px;  width: ' +
...
function get_html_mp_sprite(punit, unit_panel_pos) {
  ...
  return (
    ...
    '; margin-top:' +
    top_adj +
    'px; margin-right: -64px; float:left; content-align:left;' +
    'background: transparent url(' +
    mp_sprite['image-src'] +
    ');transform: scale(0.5); background-position:-' +
    mp_sprite['tileset-x'] +
...
function get_html_vet_sprite(punit) {
  ...
  return (
    "<div style='pointer-events: none; margin-left:-46px; margin-top:-24px; margin-right: -24px; float:left; content-align:left;" +
    'background: transparent url(' +
    vet_sprite['image-src'] +
    ');transform: scale(1.0); background-position:-' +
    vet_sprite['tileset-x'] +
    'px -' +
    vet_sprite['tileset-y'] +
    'px;  width: ' +
    vet_sprite['width'] +
    'px;height: ' +
    vet_sprite['height'] +
...
function get_html_activity_sprite(punit) {
  ...
  if (proto_sprite) {
    ...
    if (proto_sprite['key'] == 'unit.auto_settler') {
      // 96-28-68. 68/2 = 34
      ml -= 38;
      mr -= 30;
      mt = -9;
    ...
    return (
      ...
      'px; margin-right: ' +
      mr +
      'px; float:left; content-align:left;' +
      'background: transparent url(' +
      action_sprite['image-src'] +
      ');transform: scale(0.95); background-position:-' +
      action_sprite['tileset-x'] +
      'px -' +
      action_sprite['tileset-y'] +
      'px;  width: ' +
      action_sprite['width'] +
      'px;height: ' +
      action_sprite['height'] +
      'px;' +
      ' content-align: left;' +
      "vertical-align:top; float:left;'>" +
      '</div>'
...
function get_sprite_from_tag(tag) {
  ...
  return {
    tag: tag,
    'image-src':
      '/tileset/freeciv-web-tileset-' + tileset_name + '-' + i + tileset_extension + '?ts=' + ts,
    'tileset-x': tileset_x,
    'tileset-y': tileset_y,
    width: width,
    height: height,
...
function get_unit_activity_sprite(punit) {
  ...
  switch (activity) {
    ...
    case ACTIVITY_SENTRY:
      ...
      if (client_rules_flag[CRF_MP2_D]) {
        if (
          tile_has_extra(tiles[punit['tile']], EXTRA_DEEPDIVE) &&
          get_unit_class_name(punit) == 'Submarine'
        ) {
          return {
            key: 'unit.sentry_hidden',
            offset_x: unit_activity_offset_x,
  ...
  if (unit_has_goto(punit)) {
    ...
    if (
      punit['movesleft'] > 0 && // The above didn't work on injured units because they begin with less than full move points.
    ...
    if (punit['orders_vigilant'] && punit['orders_repeat']) {
      if (punit['orders_index'] / (punit['orders_length'] - 1) <= 0.5)
        return {
          key: 'unit.patrol',
          offset_x: unit_activity_offset_x,
          offset_y: -unit_activity_offset_y,
        };
      // indicates unit is on the return journey of its patrol
      else
        return {
          key: 'unit.patrol_back',
          offset_x: unit_activity_offset_x,
          offset_y: -unit_activity_offset_y,
        };
    ...
    return {
      key:
        punit['orders'][punit['orders'].length - 1]['order'] != ORDER_ACTION_MOVE
          ? 'unit.go_and'
          : 'unit.goto',
      offset_x: unit_activity_offset_x,
      offset_y: -unit_activity_offset_y,
...
function get_city_fortifications_overlay_sprite(pcity) {
  // LAYER THREE: Overlays
  if (
    pcity['walls'] & CITY_FORTIFICATIONS &&
    !(pcity['walls'] & CITY_WALLS) &&
    !(pcity['walls'] & CITY_CITADEL)
  ) {
...
function get_city_info_text(pcity) {
  return { key: 'city_text', city: pcity, offset_x: citybar_offset_x, offset_y: citybar_offset_y };
}
...
function get_tile_specials_sprite(ptile) {
  ...
  if (extra_id !== null) {
    ...
    if (extra != null) {
      ...
      if (
        tile_get_known(ptile) == TILE_KNOWN_UNSEEN &&
...
function get_tile_river_like_sprite(ptile, extra, prefix, abort_outlets) {
  ...
  } else if (client_rules_flag[CRF_CANALS]) {
    // Ruleset with Canal and Waterway
    // process in order of frequency
    if (extra == EXTRA_RIVER) {
      extra2 = EXTRA_NAVALBASE;
  ...
  if (typeof EXTRA_SEABRIDGE !== 'undefined' && extra == EXTRA_SEABRIDGE) {
    ...
    if (pterrain['graphic_str'] != 'coast') {
      for (var i = 0; i < num_cardinal_tileset_dirs; i++) {
        ...
        if (
          checktile != null &&
          (tile_has_extra(checktile, extra) || tile_has_extra(checktile, extra2))
        ) {
    ...
    // Dealing with a normal "river-ish" extra with cardinal connect keys and same type of outlet codes:
    if (
      tile_has_extra(ptile, extra) ||
      ...
      for (var i = 0; i < num_cardinal_tileset_dirs; i++) {
        ...
        if (
          ...
          (tile_has_extra(
            checktile,
            extra
          ) /*|| (clone_extra != false && tile_has_extra(checktile, clone_extra))*/ || // should be caught by the integrates we put in
...
function get_improvement_image_sprite(pimprovement) {
  ...
  return {
    ...
    'image-src':
      '/tileset/freeciv-web-tileset-' + tileset_name + '-' + i + tileset_extension + '?ts=' + ts,
    'tileset-x': tileset_x + 0.25,
    'tileset-y': tileset_y + 0.5,
    width: width,
    height: height - 1.75,
  };
}
...
function fill_road_rail_sprite_array(ptile, pcity) {
  ...
  for (var dir = 0; dir < 8; dir++) {
    ...
    if (tile1 != null && tile_get_known(tile1) != TILE_UNKNOWN) {
      ...
      if (SEABRIDGE_active) {
        if (cardinal_tileset_dirs.includes(dir)) {
          // only cardinal tiles are valid road-connectors
        ...
        // rails go on top so we catch it contrarily here:
          ...
          // non-cardinal (diagonally adjacent) tiles don't connect rails if EITHER has a sea bridge
      ...
      if (MAGLEV_active) {
        ...
        /* Same as above for rules with no EXTRA_MAGLEV:  Draw rail/road if
  ...
  if (WALLS_active && walls) {
    walls_sprite = get_tile_river_like_sprite(
      ptile,
      EXTRA_WALLS,
      'road.wall',
      true /*aborts sea outlets*/
    );
  }
...
function fill_irrigation_sprite_array(ptile, pcity) {
  ...
  if (tile_has_extra(ptile, EXTRA_IRRIGATION) && pcity == null) {
    if (tile_has_extra(ptile, EXTRA_FARMLAND)) {
      result_sprites.push({ key: tileset_extra_id_graphic_tag(EXTRA_FARMLAND) });
    } else {
      result_sprites.push({ key: tileset_extra_id_graphic_tag(EXTRA_IRRIGATION) });
    }
  }
  ...
  if (pcity == null) {
    if (typeof EXTRA_NAVALBASE !== 'undefined') {
      if (tile_has_extra(ptile, EXTRA_NAVALBASE)) {
        ...
        result_sprites.push({ key: 'base.navalbase_bg', offset_y: -normal_tile_height / 2 });
        return result_sprites;
      }
    }
    ...
    if (typeof EXTRA_CASTLE !== 'undefined') {
      if (tile_has_extra(ptile, EXTRA_CASTLE)) {
        result_sprites.push({ key: 'base.castle_bg', offset_y: -normal_tile_height / 2 });
        return result_sprites;
      }
    }
    if (tile_has_extra(ptile, EXTRA_FORTRESS)) {
      result_sprites.push({ key: 'base.fortress_bg', offset_y: -normal_tile_height / 2 });
    }
    ...
    // But we also have to check if it's defined because some rulesets don't define it
      ...
      if (tile_has_extra(ptile, EXTRA_FORT)) {
        result_sprites.push({ key: 'base.outpost_bg', offset_y: -normal_tile_height / 2 });
      }
    }
  }
  return result_sprites;
...
function fill_layer2_sprite_array(ptile, pcity) {
  ...
  if (pcity == null) {
    if (typeof EXTRA_BUNKER !== 'undefined' && tile_has_extra(ptile, EXTRA_BUNKER)) {
      result_sprites.push({ key: 'base.bunker_mg', offset_y: -normal_tile_height / 2 });
    ...
    if (tile_has_extra(ptile, EXTRA_AIRBASE)) {
      result_sprites.push({ key: 'base.airbase_mg', offset_y: -normal_tile_height / 2 });
    } else if (tile_has_extra(ptile, EXTRA_BUOY)) {
      result_sprites.push(get_base_flag_sprite(ptile));
      result_sprites.push({ key: 'base.buoy_mg', offset_y: -normal_tile_height / 2 });
    } else if (typeof EXTRA_FISHTRAP !== 'undefined' && tile_has_extra(ptile, EXTRA_FISHTRAP)) {
      result_sprites.push({ key: 'ts.fishtrap', offset_y: 0 });
    }
    if (typeof EXTRA_TILE_CLAIM !== 'undefined' && tile_has_extra(ptile, EXTRA_TILE_CLAIM)) {
      result_sprites.push({ key: 'base.tileclaim_mg', offset_y: -normal_tile_height / 2 });
      result_sprites.push(get_base_flag_sprite(ptile));
    }
    if (
    ...
    if (tile_has_extra(ptile, EXTRA_RUINS)) {
      result_sprites.push({ key: 'extra.ruins_mg', offset_y: -normal_tile_height / 2 });
    }
    if (typeof EXTRA_RADAR !== 'undefined') {
      if (tile_has_extra(ptile, EXTRA_RADAR)) {
        result_sprites.push({ key: 'base.radar_mg', offset_y: -normal_tile_height / 2 });
      }
    }
    if (typeof EXTRA_QUAY !== 'undefined') {
      if (tile_has_extra(ptile, EXTRA_QUAY)) {
        result_sprites.push({ key: 'base.quay_mg', offset_y: -normal_tile_height / 2 });
      }
    }
    if (typeof EXTRA_WATCHTOWER !== 'undefined' && tile_has_extra(ptile, EXTRA_WATCHTOWER)) {
      result_sprites.push({ key: 'base.tower_mg', offset_y: -normal_tile_height / 2 });
    }
  }
...
function fill_layer3_sprite_array(ptile, stacked, st_unit) {
  ...
  if (typeof EXTRA_CASTLE !== 'undefined') {
    if (tile_has_extra(ptile, EXTRA_CASTLE)) {
      result_sprites.push({ key: 'base.castle_fg', offset_y: -normal_tile_height / 2 });
      if (stacked) result_sprites.push(fill_stacked_in_base_sprite_array(st_unit, stacked));
      return result_sprites;
    }
  }
  // fortress on top of fort, have to check for it first then return
  if (tile_has_extra(ptile, EXTRA_FORTRESS)) {
    result_sprites.push({ key: 'base.fortress_fg', offset_y: -normal_tile_height / 2 });
    if (stacked) result_sprites.push(fill_stacked_in_base_sprite_array(st_unit, stacked));
    return result_sprites;
  }
  ...
  if (typeof EXTRA_NAVALBASE !== 'undefined') {
    if (tile_has_extra(ptile, EXTRA_NAVALBASE)) {
      result_sprites.push({ key: 'base.navalbase_fg', offset_y: -normal_tile_height / 2 });
      if (stacked) result_sprites.push(fill_stacked_in_base_sprite_array(st_unit, stacked));
      return result_sprites;
    }
  }
  ...
  if (typeof EXTRA_FORT !== 'undefined') {
    if (tile_has_extra(ptile, EXTRA_FORT)) {
      result_sprites.push({ key: 'base.outpost_fg', offset_y: -normal_tile_height / 2 });
      if (stacked) result_sprites.push(fill_stacked_in_base_sprite_array(st_unit, stacked));
      return result_sprites;
    }
  }
...
function insert_utype_into_offset_arrays(i) {
  ...
  var sx = 0,
    sy = 0; // custom shield placement
  var vx = 0,
    vy = 0; // custom vet badge placement
  var mx = -10,
    my = -19; // a "starting base value" for position of multi-unit
