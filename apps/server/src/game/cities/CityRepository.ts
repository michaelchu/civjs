...
export class CityRepository {
  ...
  async loadAll(): Promise<LoadedCity[]> {
    ...
    return records.map(record => ({
      city: {
        ...
        illness: record.illness || 0,
        illnessTrade: record.illnessTrade || 0,
        turnPlague: record.turnPlague ?? undefined,
  ...
  async save(city: CityState): Promise<void> {
    const cityData = {
      ...
      illness: city.illness || 0,
      illnessTrade: city.illnessTrade || 0,
      turnPlague: city.turnPlague ?? null,
