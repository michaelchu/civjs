...
export function setupSocketHandlers(io: Server, socket: Socket) {
  ...
  socket.on('packet', async packet => {
    // Handle reference-style turn end packet
    if (packet.pid === 'packet_player_phase_done') {
      const connection = activeConnections.get(socket.id);
      if (!connection?.userId || !connection?.gameId) {
        logger.warn('Player phase done packet from unauthenticated connection');
        return;
      }

      try {
        logger.info('Player ended turn (reference-style)', {
          playerId: connection.userId,
          gameId: connection.gameId,
          turn: packet.turn,
        });

        // Set player as having ended turn
        await gameManager.setPlayerTurnDone(connection.gameId, connection.userId, true);

        // Immediately advance turn (simplified reference-style approach)
        await gameManager.processSimpleTurnEnd(connection.gameId);

        // Broadcast turn advancement to all players in game
        io.to(`game:${connection.gameId}`).emit('turn_advanced', {
          gameId: connection.gameId,
          newTurn: packet.turn + 1,
        });

        logger.debug(`Turn advanced for game ${connection.gameId}`);
      } catch (error) {
        logger.error('Error processing turn end:', error);
      }
      return;
    }

    // Handle other packets through PacketHandler
