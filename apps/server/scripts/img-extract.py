...
parser.add_argument('-s', '--sprites', default='public/sprites', help='path to sprites directory')
parser.add_argument('-o', '--outdir', default='public/tilesets', help='path to output directory')
parser.add_argument('-j', '--jsdir', default='public/js/2dcanvas', help='path to JS output directory')
...
for tileset_id in files.keys():
  # Write tileset PNG images to tilesets directory
  tileset_file = 'tileset_spec_' + tileset_id + '.js'
  
  # Write JavaScript coordinate spec to JS directory  
  with open(path.join(js_dir, tileset_file), 'w') as f:
