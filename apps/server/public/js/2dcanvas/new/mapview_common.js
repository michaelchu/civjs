...
function center_tile_mapcanvas_2d(ptile) {
  ...
  if (!active_city) {
    // City maps need exact centering, even at poles
    if (adjusted_y > server_settings.ysize.val - 1 - polar_buffer) {
      // -1 is for 0-indexing
    ...
    if (gui_x_w > 0 && gui_y_h > 0) {
      ...
      for (var ptile_index = 0; ptile_index < ptile_count; ptile_index++) {
        ...
        if (ptile != null) {
          ...
          put_one_element(mapview_canvas_ctx, layer, null, null, pcorner, null, null, cx, cy, null);
        }
      }
    } /* ///
    if (!stop_checking) {
      ...
      if (cities[city_id]['traderoute_count'] > 0) {
        for (tr = 0; tr < cities[city_id]['traderoute_count']; tr++) {
          if (city_trade_routes && city_trade_routes[city_id] && city_trade_routes[city_id][tr]) {
            var correct_x,
              src = city_id,
              dest = city_trade_routes[city_id][tr]['partner'];
            ...
            if (dest_x < src_x) {
              // always do dest is east of src (to handle dateline wrapx)
              tmp_x = dest_x;
              dest_x = src_x;
              src_x = tmp_x;
              tmp_y = dest_y;
            ...
            if (topo_has_flag(TF_WRAPX)) {
              /* Handle Wrapx other dir creates a shorter line path */
              xs = map['xsize'];
              // figure out if opposite path across international date line is shorter
              if (dest_x > src_x) {
                // destination is "east" of source in raw value
            ...
            if (!observing) {
              if (
                cities[src].owner != null &&
                cities[src].owner != client.conn.playing.playerno &&
                cities[dest].owner != null &&
                cities[dest].owner != client.conn.playing.playerno
              ) {
            ...
            mapview_canvas_ctx.moveTo(
              from['gui_dx'] - mapview.gui_x0,
              from['gui_dy'] - mapview.gui_y0
            );
...
function put_drawn_sprites(pcanvas, canvas_x, canvas_y, pdrawn, ptile) {
  for (var i = 0; i < pdrawn.length; i++) {
    ...
    if (pdrawn[i]['key'] == 'city_text') {
      ...
      /* in JS, if (A<1) then !!A == true but A == false; Yep! It's true! 😱 */
      mapview_put_scaled_image(
        pcanvas,
        pdrawn[i]['key'],
        canvas_x + offset_x,
        canvas_y + offset_y,
...
function update_map_canvas_check() {
  ...
  try {
    if (/*renderer == RENDERER_2DCANVAS &&*/ window.requestAnimationFrame != null)
...
function update_map_slide() {
  ...
  mapview_slide['i'] = Math.floor(
    (mapview_slide['max'] * (mapview_slide['slide_time'] - elapsed)) / mapview_slide['slide_time']
  );
