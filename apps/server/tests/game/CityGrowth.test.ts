...
describe('City Population Growth', () => {
  ...
  beforeEach(async () => {
    ...
    // Create mock MapManager with productive tiles for growth testing
    mockMapManager = {
      getMapData: jest.fn().mockReturnValue({
        ...
        tiles: Array(80 * 50)
          ...
          .map((_, index) => ({
            ...
            terrain: 'grassland', // Good food terrain
            resource: null,
            special: null,
            improvement: null,
            city: null,
      ...
      getTile: jest.fn((x: number, y: number) => ({
        x,
        y,
        terrain: 'grassland', // Grassland provides 2 food
        resource: null,
        special: null,
        improvement: null,
        city: null,
  ...
  describe('Basic Growth Mechanics', () => {
    it('should grow population when city has positive food surplus', async () => {
      ...
      // Test the growth logic directly by bypassing processCityTurn
      // which calls optimizeCitizens and overrides our test values
      ...
      // Call processFoodAndGrowth directly
    ...
    it('should lose population when city starves', async () => {
      ...
      // Process food and growth directly to avoid optimizeCitizens overriding values
  ...
  describe('Growth with Different Tile Configurations', () => {
  ...
  describe('Post-Growth Mechanics', () => {
    it('should properly assign new citizen to work after growth', async () => {
      const city = await cityManager.foundCity(35, 35, 'AssignmentCity', 'player-123');

      // Set up for quick growth
      city.foodPerTurn = 5; // High surplus

      // Process turns until growth using direct growth method to avoid interference
      for (let turn = 1; turn <= 4; turn++) {
        await cityManager['processFoodAndGrowth'](city, turn);
      }

      // After growth, should have:
      expect(city.population).toBe(2);

      // Should have workable tiles assigned (if tile management is working)
      // Note: This test focuses on population growth, tile assignment depends on optimizeCitizens
      expect(city.workableTiles).toBeDefined();

      // Basic check that we have workable tiles available
      if (city.workableTiles) {
        expect(city.workableTiles.length).toBeGreaterThan(0);
      }
    });
    ...
    it('should recalculate food production after growth', async () => {
      ...
      // Force growth by setting high food surplus
      city.foodPerTurn = 10;
      city.foodStock = 19; // One turn away from growth

      // Process one turn to trigger growth
      await cityManager.processCityTurn(city.id, 1);

      const grownCity = cityManager.getCity(city.id)!;

      // After growth, food production should be recalculated
      // Population 2 consumes 4 food vs population 1 consuming 2 food
      // So net food should decrease by 2
  ...
  describe('Edge Cases', () => {
    it('should handle growth exactly at granary threshold', async () => {
      ...
      expect(currentCity.population).toBe(2);
      expect(currentCity.foodStock).toBe(0);
    });
    ...
    it('should not grow with zero food surplus even with full granary', async () => {
      ...
      // Call growth logic directly to avoid overriding test values
      await cityManager['processFoodAndGrowth'](city, 1);
