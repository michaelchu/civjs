...
describe('MapValidator', () => {
  ...
  describe('validateMap (comprehensive)', () => {
    it('should pass validation for high-quality map', () => {
      ...
      expect(result.score).toBeGreaterThan(59); // More realistic for comprehensive validation with enhanced river/parameter checks
...
function createTrulyBalancedMap(width: number, height: number): MapTile[][] {
  ...
  tiles[17][9].riverMask = 2 | 8; // East-West connection
  tiles[18][9].riverMask = 8; // West connection
  tiles[19][9].riverMask = 8; // West connection (flows to ocean)
