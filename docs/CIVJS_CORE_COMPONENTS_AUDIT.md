...
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- |
| Rulesets & content             | Buildings, cities, effects, game settings, governments, nations, technologies, terrain, units, extras, actions, requirements, styles, graphics, sound/music, scenarios. | Complete classic rules/data catalogues and Amplio2 baseline; no playable scenario library, alternate rulesets, audio asset/playback system, or localization catalogue.               | Partial — scenarios, alternate rulesets, audio, and localization remain                         |
| World & map simulation         | Map topology, terrain, oceans, rivers, resources, extras, borders, visibility/fog, map generation, starting positions, climate/biomes, pathfinding.                     | Core map model, generators, terrain/rivers/resources, borders, visibility, and A* pathfinding exist; topology, exact reference behavior, and several edge systems remain incomplete. | Partial — topology, generator parity, extras/borders, and movement/visibility edge cases remain |
...
- **Scenarios and alternate rulesets — Remaining:** CivJS packages only the classic ruleset and has no reference scenario-map library, scenario metadata/loading/version-compatibility pipeline, or playable non-classic ruleset packages.
...
- packaged scenarios and their loading/compatibility pipeline;
...
- **Map model — Partial:** Tile coordinates, terrain, oceans/continents, resources, rivers, roads, railroads, improvements, borders, and terrain transformations have runtime representations. The map access layer currently uses a fixed eight-neighbor grid and does not consistently apply the reference map topology/wrap rules; its distance helper also contains simplified wrapping behavior. Bases and territory-claiming extras are not fully represented in border ownership calculations.
- **Map generation — Partial:** CivJS has random, fractal, island, fair-islands, and fracture-oriented generation services, plus height, temperature/climate, biome, ocean, river, resource, continent, and starting-position processors. The reference generator’s exact topology-aware algorithms, polar handling, terrain-placement details, retry/validation rules, and deterministic parity are not yet demonstrated. The scenario generator is explicitly not implemented and scenario maps are not loaded from packaged files.
- **Visibility — Partial:** Per-player visible/explored sets, shared-vision providers, unit/city vision, persistence, fog-filtered snapshots, and visibility updates exist. `lastSeen` is currently synthesized when queried rather than retaining the actual observation timestamp, and topology-aware line-of-sight/visibility behavior needs parity verification.
- **Movement and spatial actions — Partial:** Terrain movement costs, A* pathfinding, accessible-tile search, roads/railroads, worker improvements, terrain transformation, airlift, paradrop, and action targeting exist in the playable loop. Pathfinding and neighbor traversal do not yet share a complete topology model, and full reference parity for unit-class movement rules, occupancy/zones, transport/embarkation, goto/order semantics, and all action-specific path constraints remains to be verified.

The World & map simulation family therefore remains `Partial`. The largest
structural gap is a shared topology-aware map abstraction used consistently by
map access, generation, visibility, distance, and pathfinding. After that,
parity work should focus on the reference generator and start-position rules,
then extras-driven borders and the remaining movement/visibility edge cases.
