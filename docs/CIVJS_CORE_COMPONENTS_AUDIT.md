...
| AI & automation                | AI player lifecycle, city planning, production, research, diplomacy, military tactics, worker automation, exploration, advisors.                                        | Complete native behavioral port of the supported classic/default AI: restart-safe lifecycle and control transfer, difficulty/traits, shared wants, production/research/government/economy/citizens, settlers/workers/exploration, military/guards/hunters/ferries/air/paradrop/diplomats/wonder helpers, space-race construction and rival response, material diplomacy and incidents, plus a read-only player-facing advisor facade. | Complete for the supported classic/default-AI baseline                             |
...
or an intentional scope limit. That target is implemented: authoritative unit
movement, ownership, and destruction events keep persistent tasks coherent;
diplomatic incidents update relationship memory at the event boundary; and the
same planner rankings are exposed read-only to human players through the Empire
Advisor. Subsystem mappings and acceptance evidence are maintained in
