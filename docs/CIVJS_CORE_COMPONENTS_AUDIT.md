...
| AI & automation                | AI player lifecycle, city planning, production, research, diplomacy, military tactics, worker automation, exploration, advisors.                                        | AI creation, recovery, turn integration, deterministic baseline production/research, adjacent combat, treaty responses, and worker/explore automation exist. The Freeciv default-AI scoring, planning, difficulty, military-logistics, diplomacy, and advisor subsystems remain to be ported; the compatibility adapter is transitional rather than the target architecture. | Partial — full default-AI parity is required                                       |
...
The required target is behavioral parity with the vendored Freeciv
classic/default AI for the supported classic ruleset. The existing
`CivJSAIAdapter` is a temporary compatibility slice, not an intentional scope
limit. Subsystem mappings and acceptance evidence are maintained in
