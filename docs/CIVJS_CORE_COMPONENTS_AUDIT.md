...
turn chronology coverage. The completion checkpoint passes 965 server unit
tests and 154 database-backed integration tests. This does not claim identical
Freeciv server-command policy, score formula weighting, or client replay UI;
those are lower-level parity and presentation concerns rather than missing
lifecycle authority.
