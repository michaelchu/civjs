...
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| Rulesets & content             | Buildings, cities, effects, game settings, governments, nations, technologies, terrain, units, extras, actions, requirements, styles, graphics, sound/music, scenarios. | Complete classic rules/data catalogues and Amplio2 baseline; no alternate playable rulesets, audio asset/playback system, or localization catalogue. Scenario gameplay is explicitly deferred, with dormant reference assets retained behind a disabled provider boundary.                                                                                  | Partial — alternate rulesets, audio, and localization remain                       |
| World & map simulation         | Map topology, terrain, oceans, rivers, resources, extras, borders, visibility/fog, map generation, starting positions, climate/biomes, pathfinding.                     | Shared topology-aware map model, all supported procedural generator modes, extras-driven borders, persistent fog memory, and authoritative movement-aware A\* pathfinding.                                                                                                                                                                                  | Implemented                                                                        |
...
| AI & automation                | AI player lifecycle, city planning, production, research, diplomacy, military tactics, worker automation, exploration, advisors.                                        | AI creation, recovery, turn integration, deterministic baseline production/research, adjacent combat, treaty responses, and worker/explore automation exist. The remaining Freeciv default-AI scoring, planning, military-logistics, diplomacy, and advisor subsystems are being ported into native CivJS services coordinated by the current orchestrator. | Partial — full default-AI parity is required                                       |
...
| Persistence & recovery         | Save/load, savegame formats, database/state persistence, replay, server restart, migration/versioning, game recovery.                                                   | Drizzle schema/migrations, fail-fast startup, durable queued actions and phase checkpoints, `GameInstanceRecoveryService`, versioned/checksummed CivJS-native replay archives, and verified PostgreSQL backup/restore tooling. Freeciv savegame compatibility is intentionally out of scope.                                                                | Partial                                                                            |
...
- **Scenarios — Deferred:** Scenario gameplay is outside the current release scope. Creation is hidden in the client and rejected explicitly by the server. A disabled provider boundary, the existing map-only loader, and ten dormant classic reference assets are retained for possible future implementation.
- **Alternate rulesets — Remaining:** No playable non-classic ruleset package is included.
...
possible ruleset-specific edge case from future parity testing. Scenario
gameplay is deferred and is not counted as a map-simulation gap.
...
- **Event and replay model — Partial:** Every processed phase and structured event is durably associated with its turn. Versioned end-of-turn checkpoints contain calendar, city, unit, research, and map state; `GameReplayService` exposes ordered actions, phases, and events and reconstructs an exact completed checkpoint through the shared version codec. Re-executing the event/action stream from an earlier checkpoint is not implemented, so arbitrary event-sourced reconstruction is not claimed.

The core turn and live-game lifecycle is `Implemented`; the event-sourced replay
subitem remains `Partial`. Timer recovery, duplicate/concurrent end-turn,
phase-failure, calendar, outcome, replay-version, and database-backed turn
chronology coverage support that assessment. New-game integration coverage
asserts the reference 50-gold treasury, an automatically selected research
target, and persisted initial state. The 20-turn socket flow verifies city gold
and science output, cumulative treasury and bulbs, authoritative per-turn
deltas sent to the UI, and preservation of treasury and research across
recovery. The completion checkpoint passes 105 client unit tests, 1,027 server
unit tests, and 154 database-backed integration tests. This does not claim
identical Freeciv server-command policy, score formula weighting, arbitrary
event-stream reconstruction, or client replay UI; those are lower-level parity
and presentation concerns rather than missing live-game lifecycle
authority.
...
**Focused evidence:** `DiplomacyManager.test.ts` covers first contact, identity protection, proposal idempotency and serialization, directional and repeated clauses, material exchange, rollback, timed transitions, teams, senate restrictions, alliance cancellation, and incidents. `DiplomacyHostilityPolicy.test.ts`, `UnitManager.test.ts`, `FreecivAIOrchestrator.test.ts`, `GameManager.espionage.test.ts`, `DiplomacyHandler.test.ts`, `NationsPanel.test.tsx`, and `GameClient.management.test.ts` cover war-only hostility, allied movement, capture and strategic-attack gating, AI targeting, covert consequences, authenticated packets, and the complete client workflow.
...
The required target is functional parity with the vendored Freeciv
classic/default AI for the supported classic ruleset. `FreecivAIOrchestrator`
currently coordinates the native port; it is not a compatibility boundary or
an intentional scope limit. Subsystem mappings and acceptance evidence are maintained in
...
- **AI breadth:** Port the complete supported-classic behavior of the Freeciv
  default AI: lifecycle and memory, difficulty/traits, want-based city and
  technology decisions, government/economic policy, settlers and workers,
  military danger/defense/targeting, guards, ferries, air and paradrop
  planning, diplomats, diplomacy, exploration, and advisors. See
  `AI_PORTING_INVENTORY.md`.
...
- **Persistence breadth:** Live games recover from normalized PostgreSQL state;
  completed checkpoints export as checksummed CivJS-native archives for replay
  and inspection. Mounting an uploaded archive as a new live game remains
  deferred, as does Freeciv savegame compatibility.
- **Alternate rulesets:** Confirm the intended scope beyond classic data, including non-classic rulesets and asset/style variants. Scenario gameplay is explicitly deferred.
