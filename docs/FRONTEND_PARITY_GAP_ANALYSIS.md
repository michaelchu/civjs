...
reference scene is also snapshotted. Overview geometry now follows the
reference contract: both clients use a rectangular map-cell raster and draw
the isometric camera footprint separately as a projected polygon. The
remembered-fog presentation difference remains explicit rather than being
hidden in a permissive image threshold.
...
The map-only pixel suite seeds a deterministic 48x48 map with the explicit
isometric topology flag (`topology_id=4`) and captures the world-map canvas and
the minimap base/overlay pair. The CivJS-only suite uses strict zero-diff
...
Freeciv server or full client. It compares the terrain-only CivJS canvas and
the rectangular overview base/viewport canvases with zero channel tolerance,
including a non-square ISO map whose source raster is CSS-resampled. The world
painter uses a small test-only adapter to map CivJS native ISO tile coordinates
into the logical coordinates consumed by the legacy painter; the overview
fixture keeps the native map-coordinate raster so it follows `overview.js`
directly. Neither production map model is changed. Hex and 3D terrain modes
are not part of this coverage.
...
| FE-002 | Isometric projection, finite edges, and wrapping            | Implemented; world projection and rectangular overview geometry have regression coverage                                              | `2dcanvas/mapview_common.js:195-239,282-291` normalizes GUI positions, map wrapping, and finite-map padding; `overview.js:50-54,104-109,139-158` selects overview tile size from map width, caps displayed axes independently, and allocates one rectangular cell per map-coordinate tile.                                | `mapTopologyGeometry.ts`, `MapRenderer.ts:237-291`, and `minimapGeometry.ts` keep the main map's ISO projection separate from the overview raster. Minimap cells use one rectangular cell per native tile, preserve the reference's independent X/Y display scales, wrapped origins follow the map-axis periods, and the projected outline uses shared GUI-to-map conversion. `tests/e2e/renderer-pixel.spec.ts` and `tests/e2e/freeciv-web-pixel.spec.ts` cover square and non-square overview geometry.                                                                                            |
...
| FE-004 | Overview viewport outline and click-to-center               | Implemented; reference geometry and pixel regression exist                                                                            | `overview.js:233-275,387-400` draws the viewport outline on a separate canvas, projects the four GUI corners into map coordinates, and maps overview clicks back to map positions.                                                                                                                                        | `minimapGeometry.ts` keeps the rectangular raster independent from the camera footprint, projects GUI viewport corners through ISO map space, and emits wrapped polygon copies; `Minimap.tsx` keeps the outline on a separate overlay canvas, redraws it through `requestAnimationFrame`, cleans it up on unmount, and emits center requests consumed by `MapCanvas`. `renderer-pixel.spec.ts` verifies the exact pale-white outline color, sloped footprint, and rectangular base snapshot.                                                                                                         |
...
| FE-008 | Animations, sprites, and visual effects                     | Partial by design                                                                                                                     | The latest pinned tree includes the reference animation/effect paths in `unit.js:238-343`, `mapview_common.js:504-519`, `mapview.js`, `control.js`, and the Amplio2 tables.                                                                                                                                               | CivJS supports movement, combat, marker, border, fog, city, unit, and path presentation effects through specialized renderers. Movement redraw scheduling, cleanup, reduced-motion cancellation, transported-unit cancellation, provider cache reloads, unit overlay composition, activity-target graphics, and selection atlas frames now have regression coverage. Walking-cycle and complete legacy animation sequencing, exact camera behavior, and feature-rich sprite pixel equality remain broader parity work; the terrain/fog scene now has a strict cross-client pixel check.              |
| FE-009 | Render-only reference snapshots and cross-client comparison | Implemented for deterministic terrain and overview geometry; feature-rich world-image parity remains intentionally scoped             | `2dcanvas/mapview_common.js:266-450` paints the world canvas; `overview.js:117-379` generates the rectangular palette overview and projected viewport outline; `libs/bmp_lib.js` creates the overview image without the full client.                                                                                      | `tests/e2e/support/freecivWebRenderHarness.ts` loads those reference modules, the pinned generated sprite table, and the three carried Amplio2 atlas sheets into an isolated page. `tests/e2e/freeciv-web-pixel.spec.ts` performs zero-diff world-terrain checks plus full displayed-pixel comparisons for the rectangular overview base and separate viewport overlay, including a non-square ISO map. The feature fixture also checks the overview raster end to end; feature-rich world-image equality remains separately scoped.                                                                 |
...
- Map-only pixel baselines for the isometric world map and minimap, including
  the sloped pale-white viewport outline; no UI-panel pixels are included.
- The render-only reference suite: strict zero-diff terrain-world comparison,
  ...
  rectangular overview base and viewport overlay, and non-square ISO overview
  bounds.
...
The focused renderer/interaction batch passed 58 tests, the targeted Chromium
rendering suite passed 4 tests, and the map-only pixel suite passed 2 strict
desktop snapshot tests. The full Playwright suite passed 16 tests with 4
mobile pixel cases intentionally skipped because the reference suite and CivJS
minimap are hidden or unsupported at that breakpoint. The repository-wide gate passed with 67 client suites (330 tests)
and 179 server suites (1,894 passing, 15 skipped). CI remains the final
post-push verification of the same change.

## Known limitations

- The strict cross-client pixel comparison intentionally covers a terrain/fog
  scene without feature sprites. The feature-rich reference scene is captured
  and reviewed as a separate snapshot because current CivJS and legacy
  freeciv-web differ in some extra-neighbor semantics and presentation layers;
  exact sprite-by-sprite cross-client equality is not yet claimed.
- Overview rendering now matches the reference pipeline for the covered
  fixtures: both clients build an integer rectangular palette raster, apply one
  displayed-size image resample, and draw the ISO camera footprint separately
  as a projected polygon. The suite compares the complete displayed base and
  overlay canvases, including a non-square map.
