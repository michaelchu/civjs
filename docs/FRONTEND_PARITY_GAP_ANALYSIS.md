...
of sprite tags required by active board renderers.
...
| FE-006 | Mouse click, goto, touch, and right-drag selection | Right-drag and core touch lifecycle coverage resolved in this audit; broader interaction remains partial | `2dcanvas/mapctrl.js:54-127` defines click/mousedown modes; `mapctrl.js:131-190` handles touch pan/long-press; `control.js:367-374` activates right rectangle selection only after both axes exceed 45 px and 200 ms. | `MapCanvas.tsx` preserves existing left/Alt drag and touch behavior, while `mapInteraction.ts:isRightDragSelectionReady` and `MapCanvas.tsx` apply the delayed right-drag gate, touch pan commit, and stationary long-press context interaction. Normal right clicks still resolve to context/tile information.                                                                                                                       |
...
The repository-wide gate passed with 65 client suites (320 tests) and 179
