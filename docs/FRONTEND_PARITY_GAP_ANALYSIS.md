...
Freeciv server or full client. It compares the terrain-only CivJS canvas with
the reference canvas at zero channel tolerance, stores reference snapshots for
the feature-rich board and rectangular overview, and compares overview terrain
colors at corresponding rectangular cell centers. The world painter uses a
small test-only adapter to map CivJS native ISO tile coordinates into the
logical coordinates consumed by the legacy painter; the overview fixture keeps
the native map-coordinate raster so it follows `overview.js` directly. Neither
production map model is changed. Hex and 3D terrain modes are not part of this
coverage.
...
| FE-002 | Isometric projection, finite edges, and wrapping            | Implemented; world projection and rectangular overview geometry have regression coverage                                              | `2dcanvas/mapview_common.js:195-239,282-291` normalizes GUI positions, map wrapping, and finite-map padding; `overview.js:139-158` allocates one rectangular cell per map-coordinate tile.                                                                                                                                | `mapTopologyGeometry.ts`, `MapRenderer.ts:237-291`, and `minimapGeometry.ts` keep the main map's ISO projection separate from the overview raster. Minimap cells use one rectangular cell per native tile, wrapped origins follow the map-axis periods, and the projected outline uses shared GUI-to-map conversion. `tests/e2e/renderer-pixel.spec.ts` adds strict desktop world-map and rectangular-minimap snapshots.                                                                                                                                                                                 |
...
| FE-009 | Render-only reference snapshots and cross-client comparison | Implemented for deterministic terrain and overview geometry; feature-image parity remains intentionally scoped                        | `2dcanvas/mapview_common.js:266-450` paints the world canvas; `overview.js:117-379` generates the rectangular palette overview and projected viewport outline; `libs/bmp_lib.js` creates the overview image without the full client.                                                                                      | `tests/e2e/support/freecivWebRenderHarness.ts` loads those reference modules, the pinned generated sprite table, and the three carried Amplio2 atlas sheets into an isolated page. `tests/e2e/freeciv-web-pixel.spec.ts` performs a zero-diff terrain comparison, stores reference feature/overview snapshots, and checks rectangular overview palette colors at corresponding cell centers. Raw feature overview pixels remain separate because marker shapes and remembered-fog opacity differ; the previous raster-geometry divergence is covered by the aligned snapshots and cell parity assertion. |
...
- The render-only reference suite: strict zero-diff terrain-world comparison,
  two reference feature snapshots, two reference overview snapshots, and
  tile-center overview palette parity.
...
- Overview geometry now matches the reference: both clients use a rectangular
  palette raster, while the ISO camera footprint is drawn separately as a
  projected polygon. The suite compares the raster palette at corresponding
  cell centers and keeps raw feature overview images as separate snapshots
  because marker shapes and remembered-fog opacity are still presentation
  differences.
