...
the minimap base/overlay pair. It uses strict zero-diff snapshots for terrain,
rivers, coast outlets, roads, rails, irrigation, resources, borders, fog, and
unit/city overlays. The minimap assertion also checks the pale-white
`rgb(200,200,255)` viewport outline for a sloped isometric footprint rather
than an axis-aligned rectangle. Hex and 3D terrain modes are not part of this
coverage.
...
| FE-004 | Overview viewport outline and click-to-center      | Implemented with an intentional geometry adaptation; pixel regression exists                                                          | `overview.js:233-275,387-400` draws the viewport outline and maps overview clicks to map coordinates.                                                                                                                                                                                                                     | `minimapGeometry.ts` projects GUI viewport corners through logical map space, producing an ISO diamond and wrapped copies; `Minimap.tsx` keeps the outline on a separate overlay canvas, redraws it through `requestAnimationFrame`, cleans it up on unmount, and emits center requests consumed by `MapCanvas`. `renderer-pixel.spec.ts` verifies the exact pale-white outline color and rejects a rectangle-like row/column footprint before snapshotting the minimap.                                                                                                                             |
...
desktop snapshot tests. The full Playwright suite passed 14 tests with the 2
mobile pixel cases intentionally skipped because the minimap is hidden at that
breakpoint. The repository-wide gate passed with 67 client suites (330 tests)
and 179 server suites (1,894 passing, 15 skipped). CI remains the final
post-push verification of the same change.

## Known limitations

- The pixel suite compares deterministic CivJS map captures against committed
  CivJS baselines; it does not yet run freeciv-web and compare two live browser
  captures. A direct cross-client capture would require a reference server or
  an injection harness for the legacy JSP client.
