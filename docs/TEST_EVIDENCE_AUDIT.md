...
## Current C2C3 evidence result (2026-08-13)
...
direct zero-diff terrain/feature, entity, wrapped-seam, transient-effect, and
minimap-raster comparisons, plus source-mapped painter, city-bar,
unit-composition, interaction, and viewport geometry tests. It does not certify
exact end-to-end C2C3 rendering: the runtime selects a generated native
Hexemplio package for C2C3's ISO-hex topology, but there is not yet an
independent native-Freeciv world/entity pixel capture. The direct browser
reference harness is square ISO and now paints finite terrain/features,
units/cities, transient effects, and an X-wrapped seam. C2C3's final `32x64`
minimap is byte-exact against an independent implementation of Freeciv's
natural `2x1` raster, and its click, camera, and outline are tested end to end.

The current repository verification scanned 268 test files and validated 361
parity declarations plus 10 CivJS stack declarations. `npm run verify` passed
68 client suites (387 tests) and 180 server suites (1,899 passed and 15
intentionally skipped). The focused desktop map-parity run passed 13 tests; the
complete desktop/mobile E2E matrix passed 21 and deliberately skipped nine
fixed-pixel cases on the mobile viewport. Both production applications built
successfully.
...
| Browser rendering, controls, and freeciv-web behavior                  | Square-ISO terrain/features, entities, wrapped seam, transients, overview, camera/outline geometry, layer order, movement/selection cadence, and key interactions have source-mapped or zero-diff reference evidence; the C2C3 minimap raster has an independent native oracle. | Canvas, HUD, transport, store, and interaction tests are broad.                                                       | **Directly certified for the enumerated map-rendering fixtures; partial for all controls and native ISO-hex world/entity pixels.** Custom UI panels are outside the pixel oracle. |
...
6. **Client compatibility has strong but bounded rendering evidence.** The
   square-ISO path uses the exact generated Amplio2 package and has direct
   zero-diff terrain/feature, entity, wrapped-seam, transient, overview, and
   camera evidence. The client also preserves topology/wrap packets and selects
   topology-compatible Hexemplio assets with source-mapped native geometry,
   layers, fog, extras, units/cities, minimap, and interaction invariants.
   Complete native-Freeciv ISO-hex world/entity pixel equality is still not
   established; custom UI panels are intentionally outside map-pixel scope.
